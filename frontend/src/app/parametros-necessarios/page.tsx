...
export default function ParametrosPadronizados() {
  ...
  const fetchParametros = async () => {
    try {
      const res = await fetch("https://api.coraxy.com.br/api/parametrosnecessarios"); // URL da API
  ...
  const criarParametro = async () => {
    if (nome && erpId !== 0) { // Verifica se erpId não é 0
      ...
      try {
        const res = await fetch("https://api.coraxy.com.br/api/parametronecessario", {
          method: "POST",
  ...
  const excluirParametro = async (id: number) => {
    try {
      await fetch(`https://api.coraxy.com.br/api/parametronecessario/delete/${id}`, {
        method: "DELETE",
      });
  ...
  const salvarEdicao = async (parametroAtualizado: Parametro) => {
    try {
      await fetch(`https://api.coraxy.com.br/api/parametronecessario/update/${parametroAtualizado.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>Parâmetros necessários</h1>
